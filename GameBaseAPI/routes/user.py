...
from GameBaseAPI.schemes.auth import RegisterSchema
from GameBaseAPI.utilities.error_function import error

register_bp = Blueprint("register_bp", __name__, description="registration")
...
@register_bp.doc(summary="creates a user", description="requires name, strong password and an email; name and email must be unique")
...
def register(data):
    ...
    if "@" not in data["email"] or "." not in data["email"] or len(data["email"]) < 6:
