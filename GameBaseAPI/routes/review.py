...
from GameBaseAPI.models import Game, Review
from sqlalchemy.exc import SQLAlchemyError, IntegrityError
from GameBaseAPI.routes import paginator
from flask import g
from GameBaseAPI.policies.reviews import *
...
from GameBaseAPI.routes.game import game_bp
from GameBaseAPI.schemes.reviews import *
...
class ReviewsCollection(MethodView):
    ...
    @game_bp.doc(summary="uploads a review", description="adds a review; game id is necessary")
    @login_required
...
class ReviewResource(MethodView):
    ...
    @game_bp.doc(summary="updates a review", description="updates a review if authorization is successful")
    @login_required
    @limiter.limit("5 per minute", key_func=lambda: g.current_user)
...
@game_bp.post('<int:game_id>/reviews/<int:review_id>/report')
@game_bp.doc(summary="reports a review", description="reports a review by its id")
@login_required
@limiter.limit("100 per hour")
def report_review(game_id, review_id):
    user = g.current_user

    review = db.session.get(Review, review_id)

    if not review:
        current_app.logger.warning("Unsuccessful review report attempt; review not found")
        return error("review not found", 404)

    authorize(can_report_review, review)

    user.reported_reviews.append(review)
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error("Database error while reporting a review", exc_info=True)
        return error("database error", 500)

    current_app.logger.info("Review reported")

    return jsonify({"message": "review reported"}), 201
