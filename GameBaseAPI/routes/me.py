...
from GameBaseAPI.routes.user import check_password
from flask_jwt_extended import get_jwt_identity
from GameBaseAPI.extensions import limiter
from GameBaseAPI.schemes.me import *
...
class CurrentUser(MethodView):
    ...
    def get(self):
        return {
            ...
            "developer_profile_id": g.current_user.developer_profile.id if g.current_user.developer_profile else None
        }, 200

    @me_bp.arguments(PutMeSchema)
    @me_bp.doc(summary="update current user", description="to update password entering old password is required")
    def put(self, data):
