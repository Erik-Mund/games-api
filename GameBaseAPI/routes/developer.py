...
from GameBaseAPI.models import Developer_profile, Game
from GameBaseAPI.routes.game import GAME_SORTS
from sqlalchemy.exc import SQLAlchemyError
from GameBaseAPI.routes import paginator
from flask import g
from GameBaseAPI.policies.developers import *
...
from GameBaseAPI.schemes.developers import *
...
@developer_bp.route('/get-my')
@developer_bp.doc(description="gets own developer profile")
@login_required
@limiter.limit("50 per minute")
def get_my():
    user = g.current_user
    if user.developer_profile is None:
        return error("user has no developer_profile", 404)
...
class DevelopersCollection(MethodView):
    ...
    @developer_bp.doc(summary="creates a developer profile", description="checks authorization and posts a developer", security=[{"BearerAuth": []}])
    @login_required
    def post(self, data):
        user = g.current_user
...
class DeveloperResource(MethodView):
    ...
    @developer_bp.doc(summary="gets a developer", description="gets a developer by its id")
    ...
    @developer_bp.doc(summary="deletes a developer", description="deletes a developer by its id")
    @login_required
...
@developer_bp.doc(summary="gets developer's game", description="gets games by its developer's id")
@limiter.limit("50 per minute")
