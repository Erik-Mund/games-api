...
def test_update_review_successful(client, session, review, game, auth_header):
    data = {
        "score": 4,
        "comment": "better now"
    }

    response = client.put(f"/games/{game.id}/reviews/{review.id}", json=data, headers=auth_header)
...
def test_review_invalid_score_rejected(client, review, game, auth_header):
    data = {
        "score": -1
    }
...
def test_update_non_existing_review(client, session, game, auth_header):
    ...
    response = client.put(f"/games/{game.id}/reviews/{invalid_review_id}", json=data, headers=auth_header)
