...
from GameBaseAPI.utilities.permissions import UserRole
...
report_table = db.Table(
    "report",
    db.Column("user_id", db.Integer, db.ForeignKey("user.id"), primary_key=True),
    db.Column("game_id", db.Integer, db.ForeignKey("game.id"), primary_key=True),
)
report_reviews_table = db.Table(
    "report_reviews",
    db.Column("user_id", db.Integer, db.ForeignKey("user.id"), primary_key=True),
...
class Game(db.Model):
    ...
    release_year = db.Column(db.Integer, default=datetime.now().year)
    ...
    reported_by = db.relationship(
        "User",
        secondary=report_table,
        back_populates="reported_games"
    )
    ...
    def to_dict(self):
        ...
        for genre in self.genres:
            if genres_to_return == "":
                genres_to_return += genre.to_dict()["name"]
            else:
                genres_to_return += ", " + genre.to_dict()["name"]
...
class Review(db.Model):
    ...
    reported_by = db.relationship(
        "User",
        secondary=report_reviews_table,
    ...
    created_at = db.Column(
        ...
        default=lambda: datetime.now(UTC), # default value when a row is created - current time by UTC
        nullable=False
    )
    ...
    updated_at = db.Column(
        db.DateTime,
        default=lambda: datetime.now(UTC),
        onupdate=lambda: datetime.now(UTC), # automatically updated when PUT is called
        nullable=False
    )
...
class User(db.Model):
    ...
    reported_games = db.relationship(
        "Game",
        secondary=report_table,
        back_populates="reported_by"
    )
    reported_reviews = db.relationship(
        "Review",
        secondary=report_reviews_table,
        back_populates="reported_by"
    )
